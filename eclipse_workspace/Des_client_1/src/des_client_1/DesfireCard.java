...
//			ISOException.throwIt((short)0xABBB);
	...
	public void getValue(APDU apdu, byte[] buffer){ 
		// this line is commented so the get command may be executed also at the master directory level
		// if(selectedDF.isMasterFile()==true)ISOException.throwIt(Util.PERMISSION_DENIED);
		receiveAPDU(apdu, buffer);
		if((byte)buffer[ISO7816.OFFSET_LC]!=1)ISOException.throwIt(Util.LENGTH_ERROR);
		byte fileID=buffer[ISO7816.OFFSET_CDATA];
		
		// FileID = 0x08 calls server applet
		if (fileID == 0x08){
			
			// obtain the server AID Object
			AID server_aid = JCSystem.lookupAID(server, (short) 0, (byte)server.length);
			if (server_aid == null)
				ISOException.throwIt(Util.WRONG_SERVER_AID);
			...
			if (sio == null)
				ISOException.throwIt(Util.FAILED_TO_OBTAIN_SIO);
